...
\begin{tikzpicture}
  ...
  % presentation level
  \node[below=of doc_json]                                (prog_dummy)            {};
  \node[program_id,left=of prog_dummy,anchor=north east]  (prog_webapp)           {{\Huge\faLaptop}\\Web app};
  \node[program_id,right=of prog_dummy,anchor=north west] (prog_app)              {{\Huge\faMobile}\\Mobile app};
  \node[document_id,below=of prog_dummy]                  (doc_pdf)               {\fileext{pdf}};
  ...
  % level labels
  \begin{scope}
    \node[row_label] at (-50mm,    1mm) {\rowlabel{Source}};
    \node[row_label] at (-50mm,  -50mm) {\rowlabel{Intermediate}};
    \node[row_label] at (-50mm, -109mm) {\rowlabel{Presentation}};
  \end{scope}
