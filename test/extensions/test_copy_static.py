...
class TestCopyStatic(TestExtension):
    ...
    def test_example(self, copyfile, *_):
        ...
        for i, (title, path) in enumerate(PATHS):
            with self.subTest((title, path)):
                ...
                self.assertEqual(asts[i][1]['c'][0]['c'][2][0],
                                 '_en/present.png')
                self.assertEqual(asts[i][2]['c'][0]['c'][0]['c'][2][0],
                                 '_en/subfolder/present.mp4')
                self.assertEqual(asts[i][3]['c'][2]['c'][2][0],
                                 '_en/localizable.gif')
                self.assertEqual(asts[i][3]['c'][3]['c'][2][0],
                                 '_en/{}example_video.ogv'.format(jpath))
                self.assertEqual(asts[i][4]['c'][2][0],
                                 'https://www.example.com/example.png')
                self.assertEqual(asts[i][5]['c'][0]['c'][1][0]['c'][2][0],
    ...
    def test_only_en_present(self, copyfile, isfile, *_):
        ...
        isfile.side_effect = itertools.cycle((
            ...
            False, True, False, True, False, True, False, True,  # la
        ))
